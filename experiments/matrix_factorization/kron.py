...
from metaheuristics import evolutionary_algorithm, directed_evolution, get_cost
...
kron_matrix = kron_def()

N = kron_matrix.size(0)
print(f"Kroneker product matrix with N = {N}")
...
for column in range(10, 81, 15):
    ...
    A_rec, right, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = model(kron_matrix.clone())
    diff_rec = torch.abs(kron_matrix - A_rec)
    loss = torch.norm(diff_rec, p = 'fro')
    results_karate['Original MMF'].append([column, loss.item()])

    # Nystrom
    print("Running Nystrom...")
    A_rec, C, W_inverse = nystrom_fps_model(kron_matrix.clone(), dim = column)
    results_karate['Nystrom'].append([column, torch.norm(kron_matrix - A_rec, p = 'fro').item()])

    # Random MMF
    print("Running Random MMF...")
    wavelet_indices, rest_indices = heuristics_random(kron_matrix.to_sparse(), L = column, K = 8, drop = 1, dim = N - column)
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(kron_matrix.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices, rest_indices = rest_indices, epochs = 64, learning_rate = 1e-4, early_stop = True)  
    results_karate['Random MMF'].append([column, torch.norm(kron_matrix - A_rec, p = 'fro').item()])

    # K neighbours 
    print("Running K neighbours MMF...")
    wavelet_indices, rest_indices = heuristics_k_neighbors_single_wavelet(kron_matrix.to_sparse(), L = column, K = 8, drop = 1, dim = N - column)
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(kron_matrix.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices, rest_indices = rest_indices, epochs = 64, learning_rate = 1e-4, early_stop = True)
    results_karate['K neighbours MMF'].append([column, torch.norm(kron_matrix - A_rec, p = 'fro').item()])
    ...
    results_karate['EA MMF'].append([column, torch.norm(kron_matrix - A_rec, p = 'fro').item()])
    ...
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(kron_matrix.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices.unsqueeze(-1).tolist(), rest_indices = rest_indices.tolist(), epochs = 64, learning_rate = 1e-4, early_stop = True)
    results_karate['DE MMF'].append([column, torch.norm(kron_matrix - A_rec, p = 'fro').item()])
...
plt.title(f'Kroneker product matrix (N = {N})', fontsize=14)
...
with open('kron_results.json', 'w') as f:
    json.dump(results_karate, f, indent=4)
    
print("Raw data saved to kron_results.json")