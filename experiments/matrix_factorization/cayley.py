...
# --- Cayley graph parameters ---
cayley_order = 3
cayley_depth = 4 

cayley_laplacian, cayley_x, cayley_y, cayley_edges = cayley_def(
    cayley_order,
    cayley_depth
)

N = cayley_laplacian.size(0)
print(f"Cayley graph with N = {N} nodes")
...
for column in range(10, 61, 10):
    print(f"\n=== Processing dimension: {column} ===")
    
    # Original MMF
    print("Running Original MMF...")
    model = Baseline_MMF(N, N - column, column) # N = L * drop + dim
    A_rec, right, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = model(cayley_laplacian.clone())
    diff_rec = torch.abs(cayley_laplacian - A_rec)
    loss = torch.norm(diff_rec, p = 'fro')
    results_karate['Original MMF'].append([column, loss.item()])

    # Nystrom
    print("Running Nystrom...")
    A_rec, C, W_inverse = nystrom_fps_model(cayley_laplacian.clone(), dim = column)
    results_karate['Nystrom'].append([column, torch.norm(cayley_laplacian - A_rec, p = 'fro').item()])

    # Random MMF
    print("Running Random MMF...")
    wavelet_indices, rest_indices = heuristics_random(cayley_laplacian.to_sparse(), L = column, K = 8, drop = 1, dim = N - column)
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(cayley_laplacian.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices, rest_indices = rest_indices, epochs = 64, learning_rate = 1e-4, early_stop = True)  
    results_karate['Random MMF'].append([column, torch.norm(cayley_laplacian - A_rec, p = 'fro').item()])

    # K neighbours 
    print("Running K neighbours MMF...")
    wavelet_indices, rest_indices = heuristics_k_neighbors_single_wavelet(cayley_laplacian.to_sparse(), L = column, K = 8, drop = 1, dim = N - column)
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(cayley_laplacian.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices, rest_indices = rest_indices, epochs = 64, learning_rate = 1e-4, early_stop = True)
    results_karate['K neighbours MMF'].append([column, torch.norm(cayley_laplacian - A_rec, p = 'fro').item()])
    ...
    wavelet_indices, rest_indices, ea_cost, ea_min_cost_per_gen, ea_mean_cost_per_gen, ea_all_time_min_cost_per_gen = evolutionary_algorithm(
        get_cost, cayley_laplacian.clone(), L = column, K = 8, 
        population_size = 100, generations = 100, mutation_rate = 0.2
    )
    print(f'EA loss {ea_all_time_min_cost_per_gen[-1]}')
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(cayley_laplacian.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices.unsqueeze(-1).tolist(), rest_indices = rest_indices.tolist(), epochs = 64, learning_rate = 1e-4, early_stop = True)
    results_karate['EA MMF'].append([column, torch.norm(cayley_laplacian - A_rec, p = 'fro').item()])
    ...
    A_rec, U, D, mother_coefficients, father_coefficients, mother_wavelets, father_wavelets = learnable_mmf_train(cayley_laplacian.clone(), L = column, K = 8, drop = 1, dim = N - column, wavelet_indices = wavelet_indices.unsqueeze(-1).tolist(), rest_indices = rest_indices.tolist(), epochs = 64, learning_rate = 1e-4, early_stop = True)
    results_karate['DE MMF'].append([column, torch.norm(cayley_laplacian - A_rec, p = 'fro').item()])
...
plt.title(f'Cayley Graph Laplacian (N = {N})', fontsize=14)
...
with open('cayley_results.json', 'w') as f:
    json.dump(results_karate, f, indent=4)
    
print("Raw data saved to cayley_results.json")